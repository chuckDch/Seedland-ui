...
```html
<template>
  <sd-button type="text" @click="open">点击打开 Message Box</sd-button>
</template>
...
如果你完整引入了 Seedland，它会为 Vue.prototype 添加如下全局方法：$msgbox, $alert, $confirm 和 $prompt。因此在 Vue instance 中可以采用本页面中的方式调用 `MessageBox`。调用参数为：
- `$msgbox(options)`
...
```javascript
import { MessageBox } from 'seedland-ui';
```
